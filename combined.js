...
//variable names originally based on the idea I may have had more graphs and globals 
//than final result, but was easier to leave them as written
...
colorScale_map = d3.scaleLinear()
    .domain([0, .35])//35% is just above the max, although far below the minimum, 0 seemed like a good visual baseline 
    .range(["white", "green"]);
...
info.update = function (feature) {
    this._div.innerHTML = '<h4>US Public Employment Density</h4>' +  (feature ?
        'In ' + feature.properties.name +' '+ (feature.data.PEPW * 100).toFixed(2) 
        + '% of the workforce </br>is employed in the public sector'
        : 'Hover over a state');
...
let state = {
  ...
  function init() {
    ...
    const legend_graph_group = svg.append("rect")//background
        .attr("class", "legend")
    ...
    svg.selectAll("legend")//legend color squares
        .data(["Democrat","Republican","No Governor"])
        .enter()
        .append("rect")
            .attr("x",(width3 - (width3/4) - margin3.right)+20)
            .attr("y", function(d,i){ return (height3 - (height3/4) - margin3.bottom) + i*(size+5)})//dynamic and equally spaced
            .attr("width", size)
            .attr("height", size)
    ...
    //tried appending this to the backround rec, but it wouldn't appear...
    //this caused floating/placement issues
        .append("div")
    ...
    selectElement
      .on("change", (event) => {
          ...
          //added these transitions to clear the svg before drawing new elements
...
/*first create the tooltips*/
    ...
    if (state.selectedGraph === "Total Workforce"){
        let xAxis = d3.axisBottom(xScale3_TWF)
            .tickFormat(d => (d / 1000000) + 'M');//conversion

    const xAxisGroup = svg.append("g")
        .attr("class", 'xAxis')
        .attr("transform", `translate(${0}, ${height3 - margin3.bottom})`)//move to bottom
        .style("position", "absolute")
        .style("z-index", 5)
    ...
    xAxisGroup.append("text")
        ...
        .attr("y", 40)
        .attr("text-anchor", "middle")
        .attr("fill", "#333333")//tried overriding the axisBottom fill attribution, but it doesn't work
        .attr("font-family", "Amiri")
        .attr("font-size", "1.5em")
